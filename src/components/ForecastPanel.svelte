  ...
  <div class="forecast-panel">
    {#each dailyTime.slice(0, daysToShow) as date, i}
      {#if i < dailyMin.length && i < dailyMax.length && i < dailyWeathercode.length}
        <WeatherCard
          date={safeCall(() => new Date(date).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' }), 'Unknown')}
          mainIcon={safeAccess(icons, [dailyWeathercode[i]], '/weather-icons/unknown.svg') || '/weather-icons/unknown.svg'}
          minC={dailyMin[i]}
          maxC={dailyMax[i]}
